...
func (s *Server) addKey(tx *sqlx.Tx, personaID int, id inet256.ID, pubKey inet256.PublicKey, privKey inet256.PrivateKey) error {
	...
	_, err = tx.Exec(`INSERT INTO persona_keys (persona_id, id, public_key, private_key) VALUES ($1, $2, $3, $4)`, personaID, id[:], pubKeyData, privKeyData)
	return err
}

func (s *Server) createPersona(tx *sqlx.Tx, name string) (int, error) {
	var intID int
	err := tx.Get(&intID, `INSERT INTO personas (name) VALUES ($1) RETURNING id`, name)
	return intID, err
}

func (s *Server) lookupPersona(tx *sqlx.Tx, name string) (int, error) {
	var personaID int
	err := tx.Get(&personaID, `SELECT id FROM personas WHERE name = $1`, name)
...
func (s *Server) getPrivateKey(tx dbGetter, localID PeerID) (inet256.PrivateKey, error) {
	var privKeyData []byte
	if err := tx.Get(&privKeyData, `SELECT private_key FROM persona_keys WHERE id = $1`, localID[:]); err != nil {
		return nil, err
	}
...
func (s *Server) setPersonaFeed(tx *sqlx.Tx, persona string, feedID owlnet.FeedID) error {
	_, err := tx.Exec(`UPDATE personas SET feed_id = $2 WHERE name = $1`, persona, feedID[:])
	return err
}
...
func (s *Server) createChannel(tx *sqlx.Tx, cid ChannelID, feedID owlnet.FeedID) (int, error) {
	...
	var intID int
	err = tx.Get(&intID, `INSERT INTO channels (persona_id, name, feed_id, oob_store_id) VALUES ($1, $2, $3, $4) RETURNING id`, personaID, cid.Name, feedID[:], oobStoreID)
	return intID, err
}
...
func (s *Server) lookupChannel(tx *sqlx.Tx, cid ChannelID) (id int, err error) {
	err = tx.Get(&id, `SELECT channels.id FROM personas
		...
		WHERE personas.name = $1 AND channels.name = $2
...
func (s *Server) lookupChannelFeed(tx *sqlx.Tx, cid ChannelID) (owlnet.FeedID, int, error) {
	...
	err := tx.Get(&x, `SELECT feeds.id, feeds.store_id FROM channels
		...
		WHERE personas.name = $1 AND channels.name = $2`, cid.Persona, cid.Name)
