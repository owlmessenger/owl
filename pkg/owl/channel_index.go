...
func (s *Server) buildChannelIndex(tx *sqlx.Tx, cid ChannelID) error {
	...
	if _, err := tx.Exec(`DELETE FROM channel_events WHERE channel_id = $1`, chanInt); err != nil {
		return err
	}
	var n uint64
	var index uint64
...
func (s *Server) putChannelMsg(tx *sqlx.Tx, intID int, p EventPath, id feeds.NodeID) error {
	_, err := tx.Exec(`DELETE FROM channel_events WHERE channel_id = $1 AND path = $2`, intID, p.Marshal())
	if err != nil {
		return err
	}
	_, err = tx.Exec(`INSERT INTO channel_events (channel_id, path, blob_id) VALUES ($1, $2, $3)`, intID, p.Marshal(), id[:])
	return err
}

func (s *Server) isBlobIndexed(tx *sqlx.Tx, intID int, id feeds.NodeID) (bool, error) {
	var x int
	if err := tx.Get(&x, `SELECT 1 FROM channel_events WHERE channel_id = $1 AND blob_id = $2`, intID, id); err != nil && !errors.Is(err, sql.ErrNoRows) {
